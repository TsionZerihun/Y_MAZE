...
#get frames and divide pont for function arguments

#calcualte_maze_points( pointA_Left, pointB_Right, pointC_Leg, micePoints, )-> (Returns SAP,AAR,SAR,%)
def calcualte_maze_points():
    width = 1
    height = 1
    midPoint = [1/2, 1/2]
    ...
    print(len(SAP))
    print(len(armVisit))
    percentageofSAP = len(SAP) / (len(armVisit) - 2) * 100
    print("{:.1f}%".format(percentageofSAP))

    return (SAR, AAR, SAP, percentageofSAP)